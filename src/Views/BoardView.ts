...
export class BoardView {
	...
	public render(data: BoardViewData, callbacks: BoardViewCallbacks) {
		...
		if (isGallery) {
			...
			const defaultColumn =
				data.columns.find((c) => c.id === EMPTY_GROUP_VALUE) || data.columns[0];
	...
	private setupSortable(el: HTMLElement, callbacks: BoardViewCallbacks) {
		Sortable.create(el, {
			...
			onMove: (evt) => {
				...
				document
					.querySelectorAll('.board-item-drop-target-top, .board-item-drop-target-bottom')
					.forEach((el) => {
						el.classList.remove(
			...
			onEnd: (evt) => {
				...
				if (itemId && newGroupId) {
					...
					const row = this.data.rows
						? this.data.rows.find((r) => r.id === newSubGroupId)
						: null;
					...
					callbacks.onCardDrop(
						itemId,
						this.data.groupPropertyId,
						groupValue as string | null | undefined,
						this.data.subGroupPropertyId,
						subGroupValue as string | null | undefined,
					);
				}
			},
		});
